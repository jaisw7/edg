...
def computeError(df, df2, mesh, vm, basis):
    ...
    if rank != root:
        ...
        return (res[0] ** (1.0 / order)) / ((mesh.xhi - mesh.xlo) * 2 * vm.L())
...
def computeError2(df, df2, mesh, vm, basis):
    ...
    xsol = np.array(
        [0.5 * (xmesh[j] + xmesh[j + 1]) + jac[j] * r2 for j in range(Ne2)]
    ).T
    time = 0.2
...
def computeError_3D(df, df2, mesh, vm, basis, dx, dx2):
    ...
    plt.scatter(x2, y2, c="k", marker=".", s=0.1)
