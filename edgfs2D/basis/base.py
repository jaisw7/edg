...
class BaseBasis(object, metaclass=ABCMeta):
    ...
    def element_geometrical_metrics(self, nodes: np.array):
        """Given nodes, compute inverse jacobian matrix, jacobian matrix and
        its determinant"""

    @abstractmethod
    def surface_geometrical_metrics(self, nodes: np.array):
        """Given element inverse jacobian matrix, compute jacobian determinant
        and normals at faces"""

    @abstractmethod
    def scale_surface_jacobian_det(
        self, surface_jac_det: np.array, element_jac_det: np.array
    ):
        """Given element and surface jacobian determinant, scale surface
    ...
    def error(
        ...
        """Given error at element solution points, compute jacobian weighted
        error"""
    ...
    def interpolate(self, element_data: np.ndarray, interp_op: np.ndarray):
        """Given element_data and interpolation operator, compute interpolated
        solution"""
