...
class DGFSMomWriterStd:
    def _compute_moments(self, coeff):
        vm = self.vm
        cv = vm.cv()
        Nv = vm.vsize()
        cw = vm.cw()
        ...
        ele_sol[:, :, 3] = np.einsum("...j,...j->...", soln, cSqr) * (
            2.0 / 3.0 * mcw * mr
        )
        ele_sol[:, :, 3] /= ele_sol[:, :, 0]
    ...
    def __init__(
        self, tcurr, im, xcoeff, coeff, vm, cfg, cfgsect, suffix=None, extn=".txt"
    ):
        self.vm = vm
        self.cfg = cfg
        ...
        self.xsol = (
            np.matmul(self.im.T, xcoeff.reshape(K, -1)).reshape(Nqr, Ne, -1)
        ) * vm.H0()
